...
new function () { // closure
    ...
    var callback = new base2.Package(this, {
        ...
        parent:  miruken,
        imports: "miruken",
        exports: "CallbackHandler,CallbackHandlerDecorator,CallbackHandlerFilter,CallbackHandlerAspect,CascadeCallbackHandler,CompositeCallbackHandler,ConditionalCallbackHandler,ObjectCallbackHandler,AcceptingCallbackHandler,ProvidingCallbackHandler,MethodCallbackHandler,InvocationOptions,CallbackResolution,HandleMethod,Expandable,getEffectivePromise,$handle,$expand,$define,$provide,$NOT_HANDLED"
    });
        ...
        when: function (constraint) {
            var when      = new _Node(constraint),
                ...
                if (callback instanceof Deferred) {
                    return when.match(callback.getCallback().constructor, Variance.Contravariant);
    ...
    function $expand(owner) {
        if (!$isClass(owner) && (typeOf(owner) !== 'object')) {
            throw new TypeError("Definitions can only be expanded on classes or instances.");
        }
    ...
    function $define(tag, variance) {
        ...
        function definition(owner, constraint, handler, removed) {
            if (!$isClass(owner) && (typeOf(owner) !== 'object')) {
                throw new TypeError("Definitions must have a class or instance owner.");
            ...
            if (definitions.hasOwnProperty(tag)) {
                ...
                var list = definitions[tag], insert;
                if (node.constraint) {
                    var indexedNode = list.getIndex(index); 
                    insert = indexedNode || list.head;
                    while (insert &&
                    ...
                    if (!indexedNode) {
                        list.addIndex(index, node)
                    }
                }
                if (!insert) {
                    list.insertAfter(node);
                } else {
                    list.insertBefore(node, insert);
                }
            } else {
                var list = new IndexedList(node);
                list.addIndex(index, node);
                definitions[tag] = list;
            }
            return function (notifyRemoved) {
                var next = list.remove(node);
                if (!list.head) {
                    delete definitions[tag];
                }
                if (list.getIndex(index) === node) {
                    if (next && next.match(node.constraint, Variance.Invariant)) {
                        list.addIndex(index, next);
                    } else {
        ...
        function _dispatch(target, owner, callback, constraint, variance, composer) {
            while (owner && (owner !== Base) && (owner !== Object)) {
                ...
                if (list && (!invariant || index)) {
                    ...
                    while (node) {
                        if (node.match(constraint, variance)) {
                            var base       = target.base,
                                baseCalled = false;
                            target.base    = function () {
                                baseCalled = true;
                                return _dispatch(target, owner, callback, constraint,
                                                 variance, composer);
                            };
                            try {
    ...
    function _matchProtocol(match, variance) {
        if (!$isFunction(match.conformsTo)) {
            ...
            return this.constraint.conformsTo(match);
        } else if (variance === Variance.Contravariant) {
            return match.conformsTo(this.constraint);
        }
        return this.constraint === match;
    }
    ...
    function _matchClass(match, variance) {
        if (match.ancestorOf !== Base.ancestorOf) {
            ...
            return (match === this.constraint) 
                || match.ancestorOf(this.constraint)
                || this.constraint.conformsTo(match);
        } else if (variance === Variance.Contravariant) {
            return (match === this.constraint) 
                || this.constraint.ancestorOf(match)
                || match.conformsTo(this.constraint);
        }
        return this.constraint === match;
    }
    ...
    function _matchRegExp(match) {
        return this.constraint.test(match);
    }
