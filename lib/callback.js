...
new function () { // closure
    ...
    function $define(tag, variance) {
        ...
        function definition(owner, constraint, handler, removed) {
            ...
            if (!$isFunction(handler)) {
                // Allow copy semantics for handlers
                if ($copy.test(handler)) {
                    var source = Modifier.unwrap(handler);
                    if (!$isFunction(source.copy)) {
                        throw new Error("$copy expects the target to have a copy method.");
                    }
                ...
                if (nodes && (!invariant || index)) {
                    ...
                    while (node) {
                        if (node.match(constraint, varianceMatch)) {
                            ...
                            break;  // stop matching if tnvariant not satisifed
                        }
                        node = node.next;
                    }
                }
    ...
    function _matchString(match) {
        return this.constraint == match;  // match be literal or String object
    }
    ...
    function _resultImplied(result) {
