...
new function () { // closure
            ...
            CallbackResolution, function (resolution, composer) {
                ...
                if (result === $NOT_HANDLED) {
                    ...
                    if (delegate && implied.match($classOf(delegate), Variance.Contravariant)) {
                        result = delegate;
                    } else if (implied.match($classOf(this), Variance.Contravariant)) {
                        result = this;
                    }
                }
                if (result !== $NOT_HANDLED) {
    ...
    function $define(tag, variance) {
        ...
        definition.dispatch = function (handler, callback, constraint, composer) {
            ...
            var result = _dispatch(delegate, delegate, callback, constraint, v, composer);
            if (result === $NOT_HANDLED) {
                result = _dispatch(handler, handler, callback, constraint, v, composer);
            }
            return result;
        };
        function _dispatch(target, owner, callback, constraint, v, composer) {
                    ...
                    while (node) {
                        if (node.match(constraint, v)) {
                            ...
                            target.base    = function () {
                                baseCalled = true;
                                return _dispatch(target, owner, callback, constraint, v, composer);
                            };
                            try {
    ...
    function _matchClass(match, variance) {
        ...
        if (constraint === match) {
            ...
            return match.prototype &&
                (constraint.prototype instanceof match || constraint.conformsTo(match));
        }
        return false;
    }
