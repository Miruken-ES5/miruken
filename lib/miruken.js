...
new function () { // closure
    ...
    var CONSTRUCTOR_FACET          = '#constructor',
        INTERCEPTOR_FACET          = '#interceptor',
        INTERCEPTOR_SELECTOR_FACET = '#interceptorSelector';
    ...
    function _buildProxy(classes, protocols, options) {
        var base  = options.baseType || classes.shift() || Base,
            ...
            constructor: function (facets) {
                        console.log("FF " + facets);
                this._interceptors = facets[INTERCEPTOR_FACET] || [];
                if (base !== Base) {
                    ctor = _proxiedMethod('constructor', this.base, base);
                    ctor.apply(this, facets[CONSTRUCTOR_FACET]);
                }
                var interceptorSelector = facets[INTERCEPTOR_SELECTOR_FACET];
                if (interceptorSelector && interceptorSelector.length > 0) {
                    this.intercetporselector = interceptorSelector[0];
                }
            },
            extend: _proxyExtender,
            getDelegate: function () { return this._delegate; },
            setDelegate: function (value) { this._delegate = value; },
            getInterceptors: function () { return this._interceptors }
        }, {
    ...
    function _proxiedMethod(key, method, source) {
        ...
        function proxyMethod () {
            ...
            if (!interceptors) {
                ...
                if (selector) {
                    interceptors = selector(source, key, interceptors);
                }
            }
            var invocation = {
