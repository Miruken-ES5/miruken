...
new function () { // closure
    ...
    var ProxyBuilder = Base.extend({
        buildProxy: function(types, options) {
            var classes   = Array2.filter(types, $isClass),
                ...
                clazz     = classes.shift();
            return _buildProxy(clazz, protocols, classes);
        }
    });

    function _buildProxy(clazz, protocols, mixins) {
        var proxy = (clazz || Base).extend(protocols.concat(mixins), {
            constructor: function () {
                var args = Array.prototype.slice.call(arguments);
                this._interceptors = args.shift() || [];
                if (clazz) {
                    ctor = _interceptedMethod('constructor', this.base, clazz);
                    this.extend("extend", _interceptedCtorExtend);
                    ctor.apply(this, args);
                }
                this.extend = _throwIfProxyInstanceExtended;
            }
        }, {
            extend: _throwIfProxyClassExtended
        });
        var prototypes     = [proxy].concat(protocols),
            proxyPrototype = proxy.prototype,
            proxied        = {};
        for (var i = 0; i < prototypes.length; ++i) {
            var prototype = prototypes[i].prototype;
            for (key in prototype) {
                if (!(proxied.hasOwnProperty(key) || (key in Base.prototype))) {
                    var member = prototype[key];
                    if ($isNothing(member) || $isFunction(member)) {
                        proxyPrototype[key] = _interceptedMethod(key, member, proxy);
                    }
                    proxied[key] = true;
                }
            }
    ...
    function _interceptedMethod(key, method, source) {
        return function () {
            var idx          = -1,
    ...
    function _interceptedCtorExtend()
    {
        var overrides,
            result = this.base.apply(this, arguments),
            clazz  = this.constructor.prototype;
        ...
        if (overrides) {
            for (method in overrides) {
                if (!(method in Base.prototype)) {
                    this[method] = _interceptedMethod(method, this[method], clazz);
                }
            }
        }
        return result;
    ...
    function _throwIfProxyClassExtended()
    {
        throw new TypeError("Proxy classes are sealed and cannot be extended.");
    }

    function _throwIfProxyInstanceExtended()
    {
        throw new TypeError("Proxy instances are sealed and cannot be extended.");
    }
