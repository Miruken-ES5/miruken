...
new function () { // closure
     ...
     */
    var InterceptorManager = ArrayManager.extend({
        constructor: function (interceptors) {
            this.base(interceptors);
        },
        mapItem: function (item) {
    ...
    var ComponentModel = Base.extend({
        constructor: function () {
            var _key, _class, _lifestyle,
                _factory, _dependencies,
                _interceptors, _interceptorSelector,
                _invariant = false;
            this.extend({
                getKey: function () {
                    return _key || _class
                ...
                getFactory: function () {
                    var factory = _factory,
                        clazz   = this.getClass();
                    if (!factory) {
                        if (_interceptors && _interceptors.length > 0) {
                            var types = [];
                            if (clazz) {
                                types.push(clazz);
                            }
                            ...
                            factory   = function () {
                                var args         = Array.prototype.slice.call(arguments),
                                    interceptors = args.splice(0, _interceptors.length);
                                args.unshift(interceptors);
                    ...
                    if ($isFunction(actions)) {
                        ...
                        if (_dependencies === undefined) {
                            ...
                            if (dependencies.length > 0) {
                                _dependencies = manager.getItems();
                            }
                        }
                    }
                    return _dependencies;
                },
                getInterceptors: function () { return _interceptors; },
                setInterceptors: function (value) {
                    if ($isSomething(value) && !(value instanceof Array)) {
                        throw new TypeError(lang.format("%1 is not an array.", value));
                    }
                    _interceptors = value ? value.slice(0) : value;
                },
                manageInterceptors: function (actions) {
                    if ($isFunction(actions)) {
                        var manager = new InterceptorManager(_interceptors);
                        actions(manager);
                        if (_interceptors === undefined) {
                            var interceptors = manager.getItems();
                            if (interceptors.length > 0) {
                                _interceptors = manager.getItems();
                            }
                        }
                    }
                    return _interceptors;
                },
                getInterceptorSelector: function () { return _interceptorSelector; },
                setInterceptorSelector: function (value) { _interceptorSelector = value; }
            });
        }
    });
   ...
   /**
    ...
    var InterceptorBuilder = Base.extend(Registration, {
        constructor: function (component, componentModel, interceptors) {
            this.extend({
                register: function(container, composer) {
                    componentModel.manageInterceptors(function (manager) {
            ...
            this.extend({
                ...
                registerHandler: function(componentModel) {
                    var key          = componentModel.getKey(),
                        ...
                        interceptors = componentModel.getInterceptors(),
                        lifestyle    = componentModel.getLifestyle() || new SingletonLifestyle;
                    if (interceptors && interceptors.length > 0) {
                        dependencies.unshift.apply(dependencies, interceptors);
                    }
        ...
        return $provide(container, key, function handler(resolution, composer) {
            ...
            return resolution.claim(handler, clazz)
                 ? lifestyle.resolve(_activate.bind(container, clazz, factory,
                                     dependencies, resolution, composer), composer)
