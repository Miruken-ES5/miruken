...
new function () { // closure
            ...
            this.extend({
                ...
                getFactory: function () {
                    var factory = _factory,
                        clazz   = this.getClass();
                    if (!factory) {
                        var interceptors = _burden[INTERCEPTOR_FACET];
                ...
                getDependencies: function (key) { 
                    return _burden[key || CONSTRUCTOR_FACET];
                },
                setDependencies: function (key, value) {
                    if (arguments.length === 1) {
                        value = key, key = CONSTRUCTOR_FACET;
                    }
                ...
                manageDependencies: function (key, actions) {
                    if (arguments.length === 1) {
                        actions = key, key = CONSTRUCTOR_FACET;
                    }
    ...
    function _makeClassFactory(clazz) {
        return function (burden) {
            return clazz.new.apply(clazz, burden[CONSTRUCTOR_FACET]);
        }
    }
        ...
        constructor: function (component, componentModel, interceptors) {
            this.extend({
                selectWith: function (selector) {
                    ...
                    componentModel.setDependencies(INTERCEPTOR_SELECTOR_FACET, [selector]);
                    return this;
                },
                toFront: function () {
                ...
                atIndex: function (index) {
                    componentModel.manageDependencies(INTERCEPTOR_FACET, function (manager) {
                        Array2.forEach(interceptors, function (interceptors) {
